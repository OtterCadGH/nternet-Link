...
networked handheld terminal.** It adds Wi-Fi, an AI chat/tutor, a camera that
...
[Discord](https://discord.gg/mB2T4Wp63e)
...
open it in any browser (it runs fully offline).

---

## Which version should I look at?

**Build [V4](v4/). It is the current, recommended generation.** The others are
here as history and reference.

| | Version | What it is | Build it? |
| --- | --- | --- | --- |
| 🟢 | **[V4](v4/)** | Rewritten firmware, protocol, and UI. Two custom PCBs: a full **camera** stick and a tiny **chat-only** board. Browser simulator, automated tests. | **Yes, start here** |
| ⚪ | [V3](v3/) | First ESP32-S3 and camera platform, with a host proxy server. | Superseded |
...
```
README.md          you are here: the project hub
docs/              shared assets (hub images)
v1/                Ti-GPT, internal mod (legacy)
   firmware/       ESP32 .ino
   calculator/     Lua app + .tns
v2/                external adapter (docs)
v3/                ESP32-S3 and camera platform
   firmware/       ESP32 .ino
   calculator/     Lua app + .tns
   server/         host proxy server
v4/                CURRENT: the full rewrite
   README.md       tutorial and build guide (start here)
...
with no calculator modification. **V3** jumped to the ESP32-S3 with a camera
and a host proxy. **V4** is the ground-up rewrite: a reliable checksummed
protocol, no API keys in the source (they live in the device's flash), support
for any OpenAI-compatible AI provider, a browser simulator, automated tests,
and two custom circuit boards.
