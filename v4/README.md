...
It adds Wi-Fi, an AI chat/tutor, a camera that reads and solves problems, and a
...
[![nternet-Link V4 GUI simulator](docs/img/simulator.png)](https://ottercadgh.github.io/nternet-Link/v4/tools/gui_simulator.html)
...
2. Press **`W`**, pick a Wi-Fi network, type any password, press **Enter**.
   (It's simulated, so any password "connects.")
3. Press **`T`**, type a question like `derivative of x^2`, press **Enter**,
   and watch the orange spark "think," then the answer stream in.
...
## The four generations (and where V4 fits)
...
| V3 | ESP32-S3 plus camera, first modular platform | Superseded by V4 |
| **V4 (this)** | Rewritten firmware, protocol, and UI, two custom PCBs | **Current** |
...
device's flash, set from the calculator), support for any OpenAI-compatible AI
provider, and a clean, modular codebase with automated tests.
...
The calculator runs a Lua app (its keyboard and screen). The device is an
ESP32 that bridges USB-serial to Wi-Fi, talks to an AI API, drives a camera,
and saves photos and chats to a microSD card. They communicate over **nlink
v1**, a small framed protocol with CRC checks and automatic retries
...
| Features | AI chat **plus camera plus microSD** | AI chat only |
| Size | about 60 x 12.5 mm stick | about 38 x 16 mm, tiny |
...
module, `nlink-stick` bare-chip) are included as build and de-risking variants.
...
A **Seeed XIAO ESP32-S3 (Sense)** gives you the S3, camera, and SD in one
...
2. Get a free AI key (for example from [Groq](https://console.groq.com/keys)).
